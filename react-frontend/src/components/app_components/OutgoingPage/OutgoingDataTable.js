...
const OutgoingDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const p_numberTemplate12 = (rowData, { rowIndex }) => <p >{rowData.secondWeight}</p>
const pTemplate13 = (rowData, { rowIndex }) => <p >{rowData.adjustment}</p>
const pTemplate14 = (rowData, { rowIndex }) => <p >{rowData.adjustedWeight}</p>
const p_numberTemplate15 = (rowData, { rowIndex }) => <p >{rowData.netWeight}</p>
...
<Column field="secondWeight" header="Second Weight" body={p_numberTemplate12} filter={selectedFilterFields.includes("secondWeight")} hidden={selectedHideFields?.includes("secondWeight")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="netWeight" header="Net Weight" body={p_numberTemplate15} filter={selectedFilterFields.includes("netWeight")} hidden={selectedHideFields?.includes("netWeight")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Outgoing Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="outgoing"            
          onUploadComplete={() => {
            setShowUpload(false); // Close the dialog after upload
          }}/>
      </Dialog>

      <Dialog header="Search Outgoing" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
