...
import outgoingFakerFactory from "./outgoingFakerFactory";
...
const OutgoingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = outgoingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("outgoing").create(elem));
...
export default connect(mapState, mapDispatch)(OutgoingFakerDialogComponent);
