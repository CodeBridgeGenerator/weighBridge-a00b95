...
import estatesFakerFactory from "./estatesFakerFactory";
...
const EstatesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = estatesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("estates").create(elem));
...
export default connect(mapState, mapDispatch)(EstatesFakerDialogComponent);
