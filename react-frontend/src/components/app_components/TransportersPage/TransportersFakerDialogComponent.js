...
import transportersFakerFactory from "./transportersFakerFactory";
...
const TransportersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = transportersFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("transporters").create(elem));
...
export default connect(mapState, mapDispatch)(TransportersFakerDialogComponent);
