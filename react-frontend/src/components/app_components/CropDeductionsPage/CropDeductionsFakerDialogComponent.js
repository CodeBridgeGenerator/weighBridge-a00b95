...
import cropDeductionsFakerFactory from "./cropDeductionsFakerFactory";
...
const CropDeductionsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = cropDeductionsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("cropDeductions").create(elem));
...
export default connect(mapState, mapDispatch)(CropDeductionsFakerDialogComponent);
