...
import incomingFakerFactory from "./incomingFakerFactory";
...
const IncomingFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = incomingFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("incoming").create(elem));
...
export default connect(mapState, mapDispatch)(IncomingFakerDialogComponent);
