...
const TicketsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            ticketNumber: _entity?.ticketNumber,block: _entity?.block,division: _entity?.division,yop: _entity?.yop,loader: _entity?.loader,bunch: _entity?.bunch,netWeight: _entity?.netWeight,
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <InputNumber id="bunch" className="w-full mb-3 p-inputtext-sm" value={_entity?.bunch} onChange={(e) => setValByKey("bunch", e.value)}  />
            </span>
            <small className="p-error">
