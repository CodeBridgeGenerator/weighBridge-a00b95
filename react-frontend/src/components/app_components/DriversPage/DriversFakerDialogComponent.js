...
import driversFakerFactory from "./driversFakerFactory";
...
const DriversFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = driversFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("drivers").create(elem));
...
export default connect(mapState, mapDispatch)(DriversFakerDialogComponent);
