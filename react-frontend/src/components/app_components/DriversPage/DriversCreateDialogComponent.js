...
const DriversCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            driverId: _entity?.driverId,driverName: _entity?.driverName,icNumber: _entity?.icNumber,licenseNumber: _entity?.licenseNumber,
        ...
        const result = await client.service("drivers").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Drivers created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Drivers" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Drivers" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="drivers-create-dialog-component">
...
export default connect(mapState, mapDispatch)(DriversCreateDialogComponent);
