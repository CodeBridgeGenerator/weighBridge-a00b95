...
import millTicketsFakerFactory from "./millTicketsFakerFactory";
...
const MillTicketsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = millTicketsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("millTickets").create(elem));
...
export default connect(mapState, mapDispatch)(MillTicketsFakerDialogComponent);
