...
import productWeightsFakerFactory from "./productWeightsFakerFactory";
...
const ProductWeightsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = productWeightsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("productWeights").create(elem));
...
export default connect(mapState, mapDispatch)(ProductWeightsFakerDialogComponent);
