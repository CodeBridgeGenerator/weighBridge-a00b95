...
import gradingsFakerFactory from "./gradingsFakerFactory";
...
const GradingsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = gradingsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("gradings").create(elem));
...
export default connect(mapState, mapDispatch)(GradingsFakerDialogComponent);
